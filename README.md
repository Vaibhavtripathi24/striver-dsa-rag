# ⚡ Striver A2Z DSA — AI Video RAG Companion

Retrieval & Timestamp Engine over Striver's complete A2Z DSA Playlist (314+ Lectures / 130+ Hours). A student asks a question in Hinglish or English and gets an AI answer **plus clickable links that jump to the exact second in Striver's lecture video**.

```
"bhai memoization aur tabulation ka difference kahan explain kiya hai?"

→ Answer: 3-4 lines, grounded only in what Striver actually explained
→ Sources:
   [1] DP 2. Climbing Stairs | Learn How to Write 1D Recurr @ 09:37   ▸ jump
   [2] DP 6. House Robber 2 | 1D DP                        @ 04:30   ▸ jump
```

Every generic RAG demo returns a blob of text. This returns *the exact moment in Striver's lecture*.
